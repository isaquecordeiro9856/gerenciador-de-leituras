...
| `PUT/PATCH` | `/usuarios/{id}` | Atualiza dados do usuário. |
| `DELETE` | `/usuarios/{id}` | Remove um usuário e seus livros associados. |
| `GET` | `/livros` | Lista todos os livros da estante (filtrar por `usuario_id`). |
| `GET` | `/livros?usuario_id={id}` | Lista livros de um usuário específico. |
| `GET` | `/livros/{id}` | Busca um livro específico. |
| `POST` | `/livros` | Cadastra um novo livro (associado ao usuário logado). |
...
Abaixo está o mapeamento das entidades que serão utilizadas na API Fake (JSON Server) e como elas se relacionam. Teremos a entidade "USUARIO", a entidade "LIVRO" e a entidade "AVALIACAO".
...
|---|---|---|---|
| USUARIO | `id` | string | Chave primária (uuid). |
| USUARIO | `nome` | string | Obrigatório, 2–100 caracteres. |
| USUARIO | `email` | string | Obrigatório. Único no sistema (RN11). Formato válido de e-mail. |
| USUARIO | `senha` | string | Obrigatório. Mínimo 6 caracteres (RN12). Deve ser armazenada de forma segura (hash). |
| USUARIO | `data_criacao` | string | Data de criação da conta (`ISO 8601`). Gerada automaticamente pelo sistema. |
| LIVRO | `id` | string | Chave primária (uuid). |
| LIVRO | `usuario_id` | string | Chave estrangeira → `USUARIO.id`. Cada livro pertence a um único usuário (RN13). |
| LIVRO | `isbn` | string | Obrigatório. Regex: 10 ou 13 dígitos numéricos. Único na estante do usuário (RN03). |
